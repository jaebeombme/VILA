...
class M3Generator:
    ...
    def __init__(self, source="local", model_path="MONAI/Llama3-VILA-M3-8B", conv_mode="llama_3"):
    ...
    def generate_response_local(
            ...
            for content in message["content"]:
                ...
                if content["type"] == "image_path":
                    ...
                    for image_path in image_paths:
                        ...
                        if image is None:
                            logger.warning(f"[WARNING] Failed to load image from path: {image_path}")
                        else:
                            images.append(image)
        ...
        # Standardize to List[Tensor]
        if images_tensor.ndim == 4:  # batched
            image_list = [img for img in images_tensor]
        elif images_tensor.ndim == 3:  # single image
            image_list = [images_tensor]
        else:
            image_list = []

        media_input = {"image": image_list} if image_list else None
        media_config = {"image": {}} if image_list else {}

        # Optional debug
        # if media_input:
        #     print(f"[DEBUG] media['image']: {[img.shape for img in media_input['image']]}")

        input_ids = (
            tokenizer_image_token(prompt_text, self.tokenizer, return_tensors="pt")
            .unsqueeze(0)
        ...
        with torch.inference_mode():
            # print(f"[DEBUG] images_tensor: {type(images_tensor)}, shape: {images_tensor.shape}")
            # print(f"[DEBUG] media['image']: {[img.shape for img in media_input['image']]}")
